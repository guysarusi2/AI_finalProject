...
# w = torch.Tensor(np.random.uniform(low=-stdv, high=stdv, size=(obs_space, 1)) * 0.003)
# v = torch.Tensor(np.random.uniform(low=-stdv, high=stdv, size=(obs_space, 1)) * 0.003)
# print(θ)
# print(w)
# print(v)
θ = torch.Tensor([[0.], [0.],[0.]])
θ.requires_grad = True
...
def run():
    ...
    for episode in range(NUM_EPISODE):
        ...
        for i in range(max_steps):
            ...
            normal_state = np.reshape(normal_state, (1, 3))
            normal_new_state = np.reshape(normal_new_state, (1, 3))

            reward = 25 * np.exp(-1 * (normal_new_state[0, 0] - 1) * (normal_new_state[0, 0] - 1) / 0.001) - 100 * np.abs(
                10 * 0.5 - (10 * 0.5 * normal_new_state[0, 0] + 0.5 * 0.3333 * normal_new_state[0, 2] * normal_new_state[
                    0, 2])) + 100 * np.abs(
                10 * 0.5 - (10 * 0.5 * normal_state[0, 0] + 0.5 * 0.3333 * normal_state[0, 2] * normal_state[0, 2]))
